...
async function getDoc() {
  if (!GOOGLE_SERVICE_ACCOUNT_KEY) throw new Error('Missing GOOGLE_SERVICE_ACCOUNT_KEY');
  
  const credentials = JSON.parse(GOOGLE_SERVICE_ACCOUNT_KEY);
  
  // Важно: Vercel часто экранирует символы \n в переменных окружения.
  // Мы заменяем их обратно на реальные переносы строк.
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    if (req.method === 'GET') {
      if (type === 'quizzes') {
        // Пытаемся найти лист Quizzes или берем первый
